...
async def create_gdrive_section(
    ...
    content_mode = data.gdrive_content_mode or "photos"

    # If URL is provided, fetch content immediately
    if data.gdrive_url and data.gdrive_url.strip():
        ...
        if content_mode == "videos":
            ...
            if not gdrive_data['videos']:
                raise HTTPException(status_code=400, detail="No videos found in the Google Drive folder. Make sure it contains video files and is publicly shared.")
            ...
            new_section = {
                ...
                "gdrive_last_sync": sync_time,
                "gdrive_error": None,
                "contributor_name": data.contributor_name,
                "contributor_role": data.contributor_role or "Videos"
            }
            sections.append(new_section)
            await db.galleries.update_one({"id": gallery_id}, {"$set": {"sections": sections}})
            ...
            for idx, video in enumerate(gdrive_data['videos']):
                gdrive_videos.append({
                    ...
                    "order": idx,
                    "is_featured": idx == 0,  # First video is featured by default
                    "synced_at": sync_time
                })
            ...
            return {"section": new_section, "video_count": len(gdrive_videos)}
        else:
            # Fetch photos (original behavior)
            ...
            return {"section": new_section, "photo_count": len(gdrive_photos)}
    else:
        ...
        new_section = {
            ...
            "order": new_order,
            "type": "gdrive",
            "gdrive_content_mode": content_mode,
...
# IMPORTANT: This route MUST be defined BEFORE the generic {section_id} route
# to prevent "reorder" from being matched as a section_id
...
async def get_contributor_upload_info(contributor_link: str):
    ...
    return {
        ...
        "section_type": section.get("type", "photo"),
        "gdrive_content_mode": section.get("gdrive_content_mode", "photos"),
        "photographer_name": photographer.get("business_name") or photographer.get("name", "Photographer"),
        ...
        "existing_gdrive_videos": existing_gdrive_videos,
...
async def submit_contributor_gdrive(contributor_link: str, data: dict = Body(...)):
    """Submit Google Drive folder URL as a contributor (photos or videos)"""
    company_name = data.get("company_name", "").strip()
    contributor_role = data.get("contributor_role", "").strip()
    ...
    if content_mode == "videos":
        ...
        final_role = contributor_role or "Videos"
        ...
        if section_idx is not None:
            ...
            sections[section_idx]["contributor_name"] = company_name
            sections[section_idx]["contributor_role"] = final_role
            await db.galleries.update_one({"id": gallery["id"]}, {"$set": {"sections": sections}})

        # Delete existing videos and re-insert
        await db.gdrive_videos.delete_many({"gallery_id": gallery["id"], "section_id": section["id"]})
        ...
        return {
            "success": True,
            "video_count": len(gdrive_videos),
            "folder_name": gdrive_data['folder_name'],
            "contributor_name": company_name,
            "contributor_role": final_role
        }

    # --- Photo mode (original behavior) ---
    final_role = contributor_role or "Photos"
